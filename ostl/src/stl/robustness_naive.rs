...
impl STLFormula {
        ...
        S: RingBufferTrait<Value = f64> + Index<usize, Output = Step<f64>>,
    {
        ...
        match self {
            ...
            STLFormula::And(phi, psi) => phi.robustness_naive(signal).min(psi.robustness_naive(signal)),

            // Or: ρ(s_t, φ ∨ ψ) = max(ρ(s_t, φ), ρ(s_t, ψ))
            STLFormula::Or(phi, psi) => phi.robustness_naive(signal).max(psi.robustness_naive(signal)),

            // Implies: ρ(s_t, φ ⇒ ψ) = max(-ρ(s_t, φ), ρ(s_t, ψ))
            STLFormula::Implies(phi, psi) => -phi.robustness_naive(signal).max(psi.robustness_naive(signal)),
            ...
            STLFormula::Until(interval, phi, psi) => {
                ...
                signal.iter()
                    ...
                    .map(|step| {
                        ...
                        let robustness_phi = signal.iter()
