    ...
    fn to_token_stream(&self) -> TokenStream2 {
        match self {
            StlFormula::True => quote! {
                ::festl::stl::formula_definition::FormulaDefinition::True
            },
            StlFormula::False => quote! {
                ::festl::stl::formula_definition::FormulaDefinition::False
            },
            StlFormula::GreaterThan(signal, val) => {
                let signal_str = signal.to_string();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::GreaterThan(#signal_str, #val as f64)
                }
            }
            StlFormula::LessThan(signal, val) => {
                let signal_str = signal.to_string();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::LessThan(#signal_str, #val as f64)
                }
            }
            StlFormula::GreaterThanVar(signal, var) => {
                ...
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::GreaterThanVar(#signal_str, #var_str)
                }
            }
            StlFormula::LessThanVar(signal, var) => {
                ...
                let var_str = var.to_string();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::LessThanVar(#signal_str, #var_str)
                }
            }
            StlFormula::Not(sub) => {
                let sub_tokens = sub.to_token_stream();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::Not(
                        Box::new(#sub_tokens)
                    )
                }
            }
            StlFormula::And(left, right) => {
                ...
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::And(
                        Box::new(#left_tokens),
            ...
            StlFormula::Or(left, right) => {
                ...
                let right_tokens = right.to_token_stream();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::Or(
                        Box::new(#left_tokens),
                        Box::new(#right_tokens)
                    )
                }
            ...
            StlFormula::Implies(left, right) => {
                ...
                let right_tokens = right.to_token_stream();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::Implies(
                        Box::new(#left_tokens),
                        Box::new(#right_tokens)
                    )
                }
            ...
            StlFormula::Globally(start, end, sub) => {
                let sub_tokens = sub.to_token_stream();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::Globally(
                        ::festl::stl::core::TimeInterval {
            ...
            StlFormula::Eventually(start, end, sub) => {
                let sub_tokens = sub.to_token_stream();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::Eventually(
                        ::festl::stl::core::TimeInterval {
                            start: ::std::time::Duration::from_secs(#start as u64),
                            end: ::std::time::Duration::from_secs(#end as u64),
                        },
            ...
            StlFormula::Until(start, end, left, right) => {
                ...
                let right_tokens = right.to_token_stream();
                quote! {
                    ::festl::stl::formula_definition::FormulaDefinition::Until(
                        ::festl::stl::core::TimeInterval {
                            start: ::std::time::Duration::from_secs(#start as u64),
                            end: ::std::time::Duration::from_secs(#end as u64),
                        },
