"""
Comprehensive test suite for festl_python.
...
class TestParseFormula:
    ...
    def test_or_keyword(self):
        ...
        formula = festl.parse_formula("x > 5 or y < 3")
        assert formula is not None
    ...
    def test_not_keyword(self):
        ...
        formula = festl.parse_formula("not(x > 5)")
        assert formula is not None
    ...
    def test_decimal_interval(self):
        ...
        formula = festl.parse_formula("G[0.5, 10.5](x > 5)")
        assert formula is not None

    def test_signal_with_underscore(self):
        """Test parsing signal names with underscores."""
        formula = festl.parse_formula("my_signal > 5")
        assert formula is not None
    ...
    def test_complex_with_monitor(self):
        ...
        formula = festl.parse_formula("G[0, 5](x > 0) && F[0, 3](y < 10)")
...
class TestFormulaCreation:
    ...
    def test_gt_formula(self):
        ...
        formula = festl.Formula.gt("x", 5.0)
        assert formula is not None
        assert "x" in str(formula)

    def test_lt_formula(self):
        """Test less-than atomic predicate."""
        formula = festl.Formula.lt("y", 10.0)
        assert formula is not None
        assert "y" in str(formula)

    def test_true_formula(self):
        """Test constant true formula."""
        formula = festl.Formula.true_()
        assert formula is not None

    def test_false_formula(self):
        """Test constant false formula."""
        formula = festl.Formula.false_()
        assert formula is not None

    def test_and_formula(self):
        """Test conjunction of formulas."""
        f1 = festl.Formula.gt("x", 1.0)
        f2 = festl.Formula.lt("x", 10.0)
        formula = festl.Formula.and_(f1, f2)
        assert formula is not None

    def test_or_formula(self):
        """Test disjunction of formulas."""
        f1 = festl.Formula.gt("x", 5.0)
        f2 = festl.Formula.lt("y", 3.0)
        formula = festl.Formula.or_(f1, f2)
        assert formula is not None

    def test_not_formula(self):
        """Test negation of formula."""
        f = festl.Formula.gt("x", 5.0)
        formula = festl.Formula.not_(f)
        assert formula is not None

    def test_implies_formula(self):
        """Test implication."""
        f1 = festl.Formula.gt("x", 0.0)
        f2 = festl.Formula.lt("y", 100.0)
        formula = festl.Formula.implies(f1, f2)
        assert formula is not None

    def test_always_formula(self):
        """Test globally (always) temporal operator."""
        f = festl.Formula.gt("x", 5.0)
        formula = festl.Formula.always(0.0, 10.0, f)
        assert formula is not None

    def test_eventually_formula(self):
        """Test eventually (finally) temporal operator."""
        f = festl.Formula.lt("x", 5.0)
        formula = festl.Formula.eventually(0.0, 5.0, f)
        assert formula is not None

    def test_until_formula(self):
        """Test until temporal operator."""
        f1 = festl.Formula.gt("x", 0.0)
        f2 = festl.Formula.gt("x", 10.0)
        formula = festl.Formula.until(0.0, 5.0, f1, f2)
        assert formula is not None

    def test_complex_formula(self):
        """Test complex nested formula."""
        # Always[0,5](x > 0.5 AND Eventually[0,2](y < 0.8))
        f1 = festl.Formula.gt("x", 0.5)
        f2 = festl.Formula.lt("y", 0.8)
        f3 = festl.Formula.eventually(0.0, 2.0, f2)
        f4 = festl.Formula.and_(f1, f3)
        formula = festl.Formula.always(0.0, 5.0, f4)
        assert formula is not None
...
class TestMonitorUpdate:
    ...
    def test_boolean_update(self):
        ...
        formula = festl.Formula.gt("x", 5.0)
        monitor = festl.Monitor(formula, semantics="DelayedQualitative")
    ...
    def test_multiple_updates(self):
        ...
        formula = festl.Formula.always(0.0, 2.0, festl.Formula.gt("x", 5.0))
        monitor = festl.Monitor(formula, semantics="DelayedQualitative")
...
class TestBatchUpdate:
    ...
    def test_batch_update_single_signal(self):
        ...
        formula = festl.Formula.gt("x", 10.0)
        monitor = festl.Monitor(formula, semantics="DelayedQualitative")
...
class TestErrorHandling:
    ...
    def test_invalid_semantics(self):
        ...
        with pytest.raises(ValueError, match="Invalid semantics"):
            festl.Monitor(formula, semantics="invalid")
    ...
    def test_invalid_algorithm(self):
        ...
        with pytest.raises(ValueError, match="Invalid algorithm"):
            festl.Monitor(formula, semantics="DelayedQualitative", algorithm="invalid")
    ...
    def test_naive_algorithm_with_eager(self):
        ...
        with pytest.raises(ValueError, match="Naive algorithm does not support"):
            festl.Monitor(formula, semantics="EagerQualitative", algorithm="Naive")
...
class TestsynchronizationStrategies:
    ...
    def test_synchronization_with_rosi(self):
        ...
        monitor_none = festl.Monitor(formula, semantics="Rosi", synchronization="None")
        assert monitor_none is not None
...
class TestTemporalFormulas:
    ...
    def test_eventually_produces_verdicts(self):
        ...
        formula = festl.Formula.eventually(0.0, 2.0, festl.Formula.gt("x", 5.0))
        monitor = festl.Monitor(formula, semantics="DelayedQualitative")
    ...
    def test_until_produces_verdicts(self):
        ...
        f1 = festl.Formula.gt("x", 0.0)
        f2 = festl.Formula.gt("x", 10.0)
        formula = festl.Formula.until(0.0, 3.0, f1, f2)
        monitor = festl.Monitor(formula, semantics="DelayedQualitative")
...
class TestBooleanOperators:
    ...
    def test_or_operator(self):
        """Test OR operator."""
        f1 = festl.Formula.gt("x", 100.0)
        f2 = festl.Formula.lt("x", 5.0)
        formula = festl.Formula.or_(f1, f2)
        monitor = festl.Monitor(formula, semantics="DelayedQualitative")
...
class TestMonitorOutputFormatting:
    ...
    def test_str_without_verdicts(self):
        ...
        formula = festl.Formula.always(0.0, 10.0, festl.Formula.gt("x", 5.0))
        monitor = festl.Monitor(formula, semantics="DelayedQuantitative")
...
class TestVariableFormulas:
    ...
    def test_parse_formula_with_variable_dollar(self):
        ...
        formula = festl.parse_formula("x > $threshold")
        assert formula is not None
    ...
    def test_formula_lt_var(self):
        ...
        formula = festl.Formula.lt_var("y", "limit")
        assert formula is not None
        assert "y" in str(formula)
...
class TestMonitorWithVariables:
    ...
    def test_get_variables_from_monitor(self):
        ...
        formula = festl.parse_formula("x > $A && y < $B")
        monitor = festl.Monitor(formula, semantics="DelayedQualitative", variables=vars)
    ...
    def test_variables_in_temporal_formula(self):
        ...
        formula = festl.parse_formula("F[0, 2](x > $limit)")
        monitor = festl.Monitor(formula, semantics="DelayedQuantitative", variables=vars)
...
class TestMonitorGetters:
    ...
    def test_get_algorithm(self):
        ...
        monitor_inc = festl.Monitor(formula, algorithm="Incremental")
        assert monitor_inc.get_algorithm() == "Incremental"

        monitor_naive = festl.Monitor(
    ...
    def test_get_semantics(self):
        ...
        monitor_rob = festl.Monitor(formula, semantics="DelayedQuantitative")
        assert monitor_rob.get_semantics() == "DelayedQuantitative"

        monitor_strict = festl.Monitor(formula, semantics="DelayedQualitative")
        assert monitor_strict.get_semantics() == "DelayedQualitative"

        monitor_eager = festl.Monitor(formula, semantics="EagerQualitative")
        assert monitor_eager.get_semantics() == "EagerQualitative"

        monitor_rosi = festl.Monitor(formula, semantics="Rosi")
    ...
    def test_get_synchronization_strategy(self):
        ...
        monitor_zoh = festl.Monitor(formula, synchronization="ZeroOrderHold")
        assert monitor_zoh.get_synchronization_strategy() == "ZeroOrderHold"

        monitor_linear = festl.Monitor(formula, synchronization="Linear")
        assert monitor_linear.get_synchronization_strategy() == "Linear"

        monitor_none = festl.Monitor(formula, synchronization="None")
    ...
    def test_get_specification(self):
        ...
        formula = festl.parse_formula("G[0,5](x > 10)")
        monitor = festl.Monitor(formula)
    ...
    def test_get_temporal_depth(self):
        ...
        formula1 = festl.parse_formula("G[0,5](x > 10)")
        monitor1 = festl.Monitor(formula1)
        assert monitor1.get_temporal_depth() == 5.0

        # Multiple temporal operators - should return the max
        formula2 = festl.parse_formula("G[0,5](x > 10) && F[0,3](y < 20)")
        monitor2 = festl.Monitor(formula2)
        assert monitor2.get_temporal_depth() == 5.0

        # Nested temporal operators
        formula3 = festl.parse_formula("G[0,10](F[0,3](x > 5))")
        monitor3 = festl.Monitor(formula3)
        assert monitor3.get_temporal_depth() == 13.0  # 10 + 3

    def test_get_signal_identifiers(self):
        """Test getting signal identifiers from a monitor."""
        # Single signal
        formula1 = festl.parse_formula("x > 5")
        monitor1 = festl.Monitor(formula1)
...
class TestMonitorDisplay:
    ...
    def test_monitor_str_basic(self):
        ...
        formula = festl.parse_formula("G[0,5](x > 10) && y < 20")
        monitor = festl.Monitor(
            formula,
            semantics="DelayedQuantitative",
            algorithm="Incremental",
    ...
    def test_monitor_str_with_variables(self):
        ...
        formula = festl.parse_formula("G[0,5](x > $threshold) && y < $limit")
        monitor = festl.Monitor(formula, semantics="DelayedQuantitative", variables=vars)
    ...
    def test_print_monitor(self):
        ...
        formula = festl.parse_formula("F[0,3](x > 5)")
        monitor = festl.Monitor(formula)
