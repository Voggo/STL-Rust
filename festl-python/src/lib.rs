...
use festl::stl::core::{RobustnessInterval, TimeInterval, Variables};
use festl::stl::formula_definition::FormulaDefinition;
use festl::stl::monitor::{
    Algorithm, DelayedQualitative, DelayedQuantitative, EagerQualitative, MonitorOutput, Rosi,
    StlMonitor,
};
use festl::stl::parse_stl;
use festl::synchronizer::SynchronizationStrategy;
use pyo3::prelude::*;
...
#[pyclass(name = "Formula", module = "festl_python.festl_python")]
#[derive(Clone)]
struct Formula {
...
impl Formula {
    ...
    fn gt(signal: String, value: f64) -> Self {
        // Leak the string to get &'static str required by festl
...
#[pyclass(name = "MonitorOutput", module = "festl_python.festl_python")]
#[derive(Clone)]
...
// -----------------------------------------------------------------------------
#[pyclass(name = "Variables", module = "festl_python.festl_python", unsendable)]
#[derive(Clone)]
struct PyVariables {
    inner: Variables,
}
...
#[pyclass(module = "festl_python.festl_python", unsendable)]
struct Monitor {
    inner: InnerMonitor,
    semantics: String,
    algorithm: String,
...
#[pymodule]
fn festl_python(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add("__doc__", "Online Signal Temporal Logic (STL) monitoring library.\n\n\
        ...
        >>> phi = festl_python.parse_formula('G[0, 5](x > 0.5)')\n\
        >>> # Create monitor with DelayedQuantitative semantics\n\
        >>> monitor = festl_python.Monitor(phi, semantics='DelayedQuantitative')\n\
        >>> # Feed data\n\
        ...
        >>> phi = festl_python.Formula.always(0, 5, festl_python.Formula.gt('x', 0.5))\n\
        >>> # Create monitor with DelayedQuantitative semantics\n\
        >>> monitor = festl_python.Monitor(phi, semantics='DelayedQuantitative')\n\
        >>> # Feed data\n\
        >>> output = monitor.update('x', 1.0, 0.0)\n\
